...
namespace TrashCollector.Data
{
    public class CustomerRepository : RepositoryBase<Customer>, ICustomerRepository
    {
        ...
        public IQueryable<Customer> GetCustomersByZipCodeAndDate(int zipCode, DateTime date) => 
            FindByConditionWithInclude(c => c.Address.ZipCode == zipCode && (c.Pickup.PickupDay == date.DayOfWeek.ToString() || c.Pickup.OneTimePickup.Date == date.Date) && c.Pickup.IsSuspended == false, a => a.Address, p => p.Pickup);
        public IQueryable<Customer> GetCustomersByZipCodeAndPickupDay(int zipCode, string day) => FindByConditionWithInclude(c => c.Address.ZipCode == zipCode && c.Pickup.PickupDay == day && c.Pickup.IsSuspended == false, a => a.Address, p => p.Pickup);
